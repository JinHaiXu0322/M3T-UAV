...
from src.datasets.m3t import M3TDataset
...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def __init__(self, args, config):
        ...
        self.train_stage = getattr(config.DATASET, "TRAIN_STAGE", "pretrain")
        ...
        self.m3t_root = getattr(config.DATASET, "M3T_ROOT", None)
        self.m3t_train_modality = getattr(config.DATASET, "M3T_TRAIN_MODALITY", "vis")
        self.m3t_test_modalities = list(getattr(config.DATASET, "M3T_TEST_MODALITIES", ["vis", "ir"]))
        self.m3t_aug_cfg = getattr(config.DATASET, "M3T_AUG", None)
        ...
        self.coarse_scale = 1 / config.LOFTDF.RESOLUTION[0]  # 0.125. for training loftr.
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit':
            if self.train_stage == 'pretrain':
                self.train_dataset = self._setup_m3t_dataset(mode='train')
            else:
            ...
            if self.train_stage == 'pretrain':
                self.val_dataset = self._setup_m3t_dataset(mode='test')
            elif isinstance(self.val_list_path, (list, tuple)):
                self.val_dataset = []
            ...
            self.test_dataset = self._setup_m3t_dataset(mode='test')
            logger.info(f'[rank:{self.rank}]: Test Dataset loaded!')

    def _setup_m3t_dataset(self, mode='train'):
        # NEW: M3T dataset supports vis->map / ir->map.
        root = self.m3t_root or self.train_data_root or self.test_data_root
        ...
        if mode == 'test':
            datasets = [
                M3TDataset(
                    ...
                    df=self.mgdpt_df,
                    augment_cfg=self.m3t_aug_cfg,
                    enable_augmentation=False,
                )
                for md in self.m3t_test_modalities
            ]
        elif self.m3t_train_modality == 'both':
            datasets = [
                M3TDataset(
                    root,
                    split=split,
                    ...
                    df=self.mgdpt_df,
                    augment_cfg=self.m3t_aug_cfg,
                    enable_augmentation=bool(self.train_stage == 'pretrain' and mode == 'train' and self.m3t_aug_cfg.ENABLE),
                ),
                M3TDataset(
                    root,
                    split=split,
            ...
            datasets = [
                M3TDataset(
                    root,
                    split=split,
                    modality=self.m3t_train_modality,
                    img_resize=self.mgdpt_img_resize,
                    df=self.mgdpt_df,
                    augment_cfg=self.m3t_aug_cfg,
                    enable_augmentation=bool(self.train_stage == 'pretrain' and mode == 'train' and self.m3t_aug_cfg.ENABLE),
    ...
    def _build_concat_dataset(
        ...
        for npz_name in tqdm(npz_names,
            ...
            # `MegaDepthDataset` loads all data from npz_path when initialized, which might take time.
    ...
    def train_dataloader(self):
        """Build training dataloader for M3T / MegaDepth."""
    ...
    def val_dataloader(self):
        """Build validation dataloader for M3T / MegaDepth."""
